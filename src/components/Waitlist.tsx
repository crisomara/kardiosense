...
const Waitlist = () => {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Send waitlist signup details via email
    const subject = `Kardiosense Waitlist Signup — ${data.name}`;
    const body = `Name: ${data.name}%0D%0AEmail: ${data.email}%0D%0ARole: ${data.role}%0D%0ACountry: ${data.country}%0D%0AMessage: ${data.message || "N/A"}`;
    window.location.href = `mailto:mail.cardiosense@gmail.com?subject=${subject}&body=${body}`;
  };
